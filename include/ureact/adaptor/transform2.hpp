...
#ifndef UREACT_ADAPTOR_TRANSFORM_2_HPP
#define UREACT_ADAPTOR_TRANSFORM_2_HPP
...
class event_transform2_node final : public event_stream_node<OutE>
{
public:
    template <typename F>
    event_transform2_node( const context& context,
        const events<InE>& source,
        F&& func,
        const signal_pack<Deps...>& deps )
        : event_transform2_node::event_stream_node( context )
        , m_source( source )
    ...
    ~event_transform2_node() override
    {
...
struct Transform2Adaptor : SyncedAdaptorBase<Transform2Adaptor<EIn>>
{
    /*!
